...
export interface GitHubController {
  ...
  authenticateUserWithOctokit(req: Request, res: Response) : Promise<Response<any, Record<string, any>> | undefined>;
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getUserInformation(req: Request, res: Response) : Promise<Response<any, Record<string, any>> | undefined>;
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  commitChangesAndCreatePR (req: Request, res: Response) : Promise<Response<any, Record<string, any>> | undefined>;
}

export class GitHubControllerImpl implements GitHubController {
   ...
   * @returns A JSON response containing the authentication token.
   */
  async authenticateUserWithOctokit(req: Request, res: Response) {
    ...
    const authentication = await authenticateWithOAuth(this.config.gitHub.clientId, this.config.gitHub.clientSecret, code as string);
   ...
   *          error message will be returned.
   */
  async commitChangesAndCreatePR (req: Request, res: Response) {
    if (!req.headers.authorization) {
      return res.status(403).json({ error: 'No credentials sent!' });
    }
    ...
    const response = await pushChangesAndCreatePullRequest(octokit, owner, repo, newOwner, branch, newBranch, this.config.git.committerName, this.config.git.committerEmail, commitMessage, change, title, body);
